  ...
  class signUpMethods {
    ...
    static authRedirecting() {
      window.setTimeout(() => {
        window.location.replace('https://mescecanteen.netlify.app/client-side.html');
      }, 500)
    }
    ...
    firebaseAuthRedirect(){
      firebase.auth().onAuthStateChanged(function(user) {
        ...
        user ?  window.location.replace('https://mescecanteen.netlify.app/client-side.html') : console.log('none');
      });
    }
  }
      ...
      logout.forEach(btn => {
        btn.addEventListener('click', (e) => {
          ...
          firebase.auth().signOut().then(() => {
            window.location.replace("https://mescecanteen.netlify.app/index.html")
          });
        });
      })
    }
